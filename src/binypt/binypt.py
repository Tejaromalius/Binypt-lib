...
class Binypt:
    ...
    """
        ...
        The Binypt class is a Python implementation designed for retrieving and processing cryptocurrency market data from the Binance exchange.
        It provides functionality for downloading, processing, and storing historical price data for a specified cryptocurrency trading pair.
        The code is released under the MIT License, allowing users to modify and distribute it as needed.
    ...
    def __init__(
        ...
        # TODO: implement a validation method for date arguments
        self.starting_date = (int(datetime.datetime.strptime(starting_date, "%d/%m/%Y-%H:%M:%S").timestamp()) * Binypt.MILISECONDS)
        self.ending_date = (int(datetime.datetime.strptime(ending_date, "%d/%m/%Y-%H:%M:%S").timestamp())* Binypt.MILISECONDS)

        # TODO: implement a validation method for output path
        self.output_path = output_path
        self.metadata = self._importMetadata(metadata_file_path)

        self.data = pd.DataFrame(columns=self.metadata.get("chart_default_columns"), dtype=float)
    ...
    def _downloadData(self):
        api_url = (f"https://api.binance.com/api/v3/klines?symbol={self.trading_pair}&interval={self.interval}&limit=1000&" + "startTime={}&endTime={}")
        bar = Bar(f"Downloaded: ", max=self.total_timelines, suffix=" retrieved %(index)d/%(max)d")
        ...
        def retreiveBatchedData(batch, batch_ix):
            while True:
                ...
                try:
                    urls = list(api_url.format(timeline[0], timeline[1]) for timeline in batch)
                    api_requests = list(thread_pool.submit(lambda url: requests.get(url).json(), (url)) for url in urls)
                    ...
                    return list(request.result() for request in api_requests if len(request.result()) != 0)

                except Exception as error:
                    continue

        for batch_ix, batch in enumerate(self.batched_timelines):
            binance_data = np.array([data for batched_data in retreiveBatchedData(batch, batch_ix) for data in batched_data])
            binance_data_df = pd.DataFrame(binance_data, columns=self.metadata.get("chart_default_columns")).astype(float)
    ...
    def _addHRTime(self):
        self.data["open_time_str"] = [datetime.datetime.fromtimestamp(time_record / Binypt.MILISECONDS) for time_record in self.data["open_time"]]
        self.data["close_time_str"] = [datetime.datetime.fromtimestamp(time_record / Binypt.MILISECONDS) for time_record in self.data["close_time"]]